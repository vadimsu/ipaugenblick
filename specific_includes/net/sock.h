...
#include <specific_includes/linux/list.h>
#include <specific_includes/linux/list_nulls.h>
...
#include <specific_includes/linux/skbuff.h>	/* struct sk_buff */
//#include <linux/mm.h>
//#include <linux/security.h>
//#include <linux/slab.h>
//#include <linux/uaccess.h>
#include <specific_includes/linux/memcontrol.h>
#include <specific_includes/linux/res_counter.h>
...
//#include <specific_includes/asm-generic/atomic-long.h>
...
#define hlist_nulls_del_init_rcu(n)     hlist_nulls_del_init(n)
#define hlist_add_head_rcu(a,b) hlist_add_head(a,b)
#define hlist_nulls_for_each_entry_rcu(a,b,c,d) hlist_nulls_for_each_entry(a,b,c,d)*/
#define sk_del_node_init_rcu(sk)	sk_del_node_init(sk)
 ...
 */
#define sock_owned_by_user(sk)	/*((sk)->sk_lock.owned)*/0
...
{
	return /*net_gso_ok(sk->sk_route_caps, sk->sk_gso_type)*/1;
}
...
#else
	return skb_add_data(skb,from,copy);
#endif
}
...
static inline int skb_copy_to_page_nocache(struct sock *sk, struct  rte_mbuf *from,
					   struct sk_buff *skb,
					   struct page *page,
					   int off, int copy)
{
	...
	err = skb_do_copy_data_nocache(sk, skb, from, page_address(page->mbuf) + off,
				       copy, skb->len);
	if (err)
		return err;
...
{
	return /*in_softirq() ? GFP_ATOMIC : GFP_KERNEL*/0;
}
...
#define LIMIT_NETDEBUG(fmt, args...) \
	do { if (net_msg_warn && net_ratelimit()) printf(fmt,##args); } while(0)
