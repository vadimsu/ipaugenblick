...
#include <specific_includes/net/netfilter/nf_log.h>
...
//		if (!strnicmp(str_logger, t->name, strlen(t->name)))
		if (!strncmp(str_logger, t->name, strlen(t->name)))
			return t;
	}
...
int nf_log_register(u_int8_t pf, struct nf_logger *logger)
{
	...
	for (i = 0; i < ARRAY_SIZE(logger->list); i++)
		LINUX_INIT_LIST_HEAD(&logger->list[i]);
...
int __init netfilter_log_init(void)
{
	...
	for (i = NFPROTO_UNSPEC; i < NFPROTO_NUMPROTO; i++)
		LINUX_INIT_LIST_HEAD(&(nf_loggers_l[i]));
