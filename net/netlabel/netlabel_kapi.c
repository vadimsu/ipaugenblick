...
static int __init netlbl_init(void)
{
	...
	printf("NetLabel: Initializing\n");
	printf("NetLabel:  domain hash size = %u\n",
	       (1 << NETLBL_DOMHSH_BITSIZE));
	printf("NetLabel:  protocols ="
	       " UNLABELED"
	       " CIPSOv4"
	       "\n");
	...
	if (ret_val != 0)
		goto init_failure;
	printf("NetLabel:  unlabeled traffic allowed by default\n");

	return 0;

init_failure:
	while(1)
		printf("NetLabel: failed to initialize properly (%d)\n", ret_val);
}
