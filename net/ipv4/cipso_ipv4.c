...
#include <specific_includes/net/netlabel.h>
#include <specific_includes/net/cipso_ipv4.h>
...
static LINUX_LIST_HEAD(cipso_v4_doi_list);
...
static int cipso_v4_cache_init(void)
{
	...
	for (iter = 0; iter < CIPSO_V4_CACHE_BUCKETS; iter++) {
		...
		LINUX_INIT_LIST_HEAD(&cipso_v4_cache[iter].list);
	}
		     ...
		     struct netlbl_audit *audit_info)
{
	...
	list_add_tail(&doi_def->list, &cipso_v4_doi_list);
	spin_unlock(&cipso_v4_doi_list_lock);
	ret_val = 0;
		     ...
		     void *cb_arg)
{
	...
	rcu_read_lock();
	list_for_each_entry(iter_doi, &cipso_v4_doi_list, list)
...
static int __init cipso_v4_init(void)
{
	...
	if (ret_val != 0)
	{
		while(1)
		{
			printf("Failed to initialize the CIPSO/IPv4 cache (%d)\n",
		      ret_val);
		}
	}
	return 0;
