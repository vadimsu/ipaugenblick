...
//#include <specific_includes/linux/ipsec.h>
//#include <asm/unaligned.h>
#include <specific_includes/linux/unaligned/le_byteshift.h>
#include <specific_includes/linux/unaligned/be_byteshift.h>
#include <specific_includes/linux/log2.h>
#include <specific_includes/asm-generic/div64.h>
#include <specific_includes/linux/math64.h>
//#include <net/netdma.h>

int sysctl_tcp_timestamps __read_mostly = 1;
int sysctl_tcp_window_scaling __read_mostly = 1;
int sysctl_tcp_sack __read_mostly = 1;
int sysctl_tcp_fack __read_mostly = 0;
int sysctl_tcp_reordering __read_mostly = TCP_FASTRETRANS_THRESH;
EXPORT_SYMBOL(sysctl_tcp_reordering);
int sysctl_tcp_dsack __read_mostly = 0;
...
int sysctl_tcp_max_orphans __read_mostly = /*NR_FILE*/0xffff;
...
int tcp_send_rcvq(struct sock *sk, struct msghdr *msg, size_t size)
{
	...
	if (memcpy_fromiovec(skb, msg->msg_iov, size))
		goto err_free;
...
restart:
	...
	while (before(start, end)) {
		...
		while (copy > 0) {
			...
			if (size > 0) {
				size = min(copy, size);
				if (skb_copy_bits(skb, offset, nskb, size))
					BUG();
...
static void tcp_urg(struct sock *sk, struct sk_buff *skb, const struct tcphdr *th)
{
	...
	if (tp->urg_data == TCP_URG_NOTYET) {
		...
		if (ptr < skb->len) {
			u8 tmp;
#if 0
			if (skb_copy_bits(skb, ptr, &tmp, skb,1))
				BUG();
#else
			skb_copy_from_linear_data_offset(skb,ptr,&tmp,1);
#endif
