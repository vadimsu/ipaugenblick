...
#include <specific_includes/linux/igmp.h>
#include <specific_includes/linux/netfilter.h>
#include <specific_includes/linux/netfilter_ipv4.h>
#include <specific_includes/linux/netfilter_bridge.h>
//#include <linux/mroute.h>
#include <specific_includes/linux/netlink.h>
...
slow_path:
	...
	while (left > 0) {
		 ...
		 */
		if (skb_copy_bits2(skb, ptr, skb_transport_header(skb2), len))
			BUG();
...
ip_generic_getfrag(void *from, char *to, int offset, int len, int odd, struct sk_buff *skb)
{
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL) {
		if (memcpy_fromiovecend(skb, iov, offset, len) < 0)
			return -EFAULT;
	} else {
		__wsum csum = 0;
		if (csum_partial_copy_fromiovecend(skb, iov, offset, len, &csum) < 0)
			return -EFAULT;
...
static inline int does_protocol_use_flat_buf(int protocol_number)
{
...
#else
			/* this is ugly, but now it works. Normal solution is required */
			if(!does_protocol_use_flat_buf(sk->sk_protocol))
			    fraglen = fragheaderlen+transhdrlen;/* because we fill skb frag */
			else
				fraglen = datalen + fragheaderlen; /* if not UDP (ICMP), copy directly to skb->data */
#endif
			if ((flags & MSG_MORE) &&
...
//				get_page(pfrag->page);
			...
			if (getfrag(from,
				    page_address(pfrag->page.mbuf) + pfrag->offset,
			    ...
			    unsigned int flags)
{
	...
	err = __ip_append_data(sk, fl4, &queue, &cork,
			       /*&current->task_frag*/sk_page_frag(sk), getfrag,
			       from, length, transhdrlen, flags);
	if (err) {
