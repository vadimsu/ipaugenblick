...
struct rte_ring *command_ring[MAXCPU];
struct rte_ring *selectors_ring[MAXCPU];
struct rte_mempool *free_connections_pool[MAXCPU];
struct rte_ring *free_connections_ring[MAXCPU];
struct rte_ring *free_clients_ring[MAXCPU];
struct rte_mempool *selectors_pool[MAXCPU];
struct rte_ring *rx_mbufs_ring[MAXCPU];
struct rte_mempool *free_command_pool[MAXCPU];
socket_satelite_data_t socket_satelite_data[MAXCPU][IPAUGENBLICK_CONNECTION_POOL_SIZE];
ipaugenblick_socket_t *g_ipaugenblick_sockets[MAXCPU];
ipaugenblick_selector_t *g_ipaugenblick_selectors[MAXCPU];
//unsigned long app_pid = 0;

TAILQ_HEAD(buffers_available_notification_socket_list_head, socket) buffers_available_notification_socket_list_head[MAXCPU];

struct ipaugenblick_clients ipaugenblick_clients[MAXCPU][IPAUGENBLICK_CLIENTS_POOL_SIZE];
TAILQ_HEAD(ipaugenblick_clients_list_head, ipaugenblick_clients) ipaugenblick_clients_list_head[MAXCPU];
...
static void on_client_connect(int client_idx)
{
	...
	*data = IPAUGENBLICK_NEW_IFACES;
	...
	rte_ring_enqueue(ipaugenblick_clients[rte_lcore_id()][client_idx].client_ring,(void *)buffer);
	buffer = get_buffer();
	if(!buffer) {
		return;
	}
...
//printf("%s %d %p %d %d %d\n",__FILE__,__LINE__,&g_ipaugenblick_sockets[socket_satelite_data->ringset_idx],socket_satelite_data->ringset_idx, rte_pktmbuf_data_len(mbuf),rte_mbuf_refcnt_read(mbuf));
                       user_increment_socket_tx_space(&g_ipaugenblick_sockets[rte_lcore_id()][socket_satelite_data->ringset_idx].tx_space,rte_pktmbuf_data_len(mbuf));
               }
        }
...
static inline void process_commands()
{
    ...
    switch(cmd->cmd) {
        case IPAUGENBLICK_OPEN_SOCKET_COMMAND:
           ...
           if(sock) {
               ...
               socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].ringset_idx = cmd->ringset_idx;
               socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].parent_idx = cmd->parent_idx;
	       socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].apppid = cmd->u.open_sock.pid;
               app_glue_set_user_data(sock,(void *)&socket_satelite_data[rte_lcore_id()][cmd->ringset_idx]);
               socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket = sock;
	       ipaugenblick_log(IPAUGENBLICK_LOG_DEBUG,"%d setting tx_space %d\n",__LINE__,sk_stream_wspace(sock->sk));
	       user_set_socket_tx_space(&g_ipaugenblick_sockets[rte_lcore_id()][socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].ringset_idx].tx_space,sk_stream_wspace(sock->sk));
           }
           ipaugenblick_log(IPAUGENBLICK_LOG_DEBUG,"Done\n");
           break;
	case IPAUGENBLICK_SOCKET_CONNECT_BIND_COMMAND:
           if(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket) { 
               if(cmd->u.socket_connect_bind.is_connect) {
			ipaugenblick_log(IPAUGENBLICK_LOG_DEBUG,"connect %x\n",cmd->ringset_idx);
		       if(app_glue_v4_connect(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket,
			   ...
			   len = sizeof(addr);
			   inet_getname(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket,&addr,&len,0);
			   g_ipaugenblick_sockets[rte_lcore_id()][cmd->ringset_idx].local_ipaddr = addr.sin_addr.s_addr;
			   g_ipaugenblick_sockets[rte_lcore_id()][cmd->ringset_idx].local_port = addr.sin_port;
               	       } 
	       }
	       else {
			ipaugenblick_log(IPAUGENBLICK_LOG_DEBUG,"bind %x\n",cmd->ringset_idx);
		       if(app_glue_v4_bind(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket,
		   		     cmd->u.socket_connect_bind.ipaddr,
				     cmd->u.socket_connect_bind.port)) {
	...
	case IPAUGENBLICK_LISTEN_SOCKET_COMMAND:
           ...
           if(app_glue_v4_listen(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket)) {
...
//	printf("%s %d %p\n",__FILE__,__LINE__,socket_satelite_data[cmd->ringset_idx].socket);
	//	user_on_transmission_opportunity(socket_satelite_data[cmd->ringset_idx].socket);
		user_flush_rx_tx((struct socket *)socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket);
		app_glue_close_socket((struct socket *)socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket);
		socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket = NULL;
		socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].ringset_idx = -1;
		socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].parent_idx = -1;
    ...
    //           user_data_available_cbk(socket_satelite_data[cmd->ringset_idx].socket);
               user_on_transmission_opportunity(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket);
           }
           break;
        case IPAUGENBLICK_SOCKET_RX_KICK_COMMAND:
           if(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket) {
               user_kick_rx++;
               user_data_available_cbk(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket);
      //         user_on_transmission_opportunity(socket_satelite_data[cmd->ringset_idx].socket);
           }
           break;
	   ...
	   if(cmd->parent_idx != -1)
	           socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].parent_idx = cmd->parent_idx;
	   socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].apppid = cmd->u.set_socket_ring.pid;
           app_glue_set_user_data(cmd->u.set_socket_ring.socket_descr,&socket_satelite_data[rte_lcore_id()][cmd->ringset_idx]);
           socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket = cmd->u.set_socket_ring.socket_descr; 
	   ...
	   user_set_socket_tx_space(&g_ipaugenblick_sockets[rte_lcore_id()][socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].ringset_idx].tx_space,
				sk_stream_wspace(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket->sk));
//           user_on_transmission_opportunity(socket_satelite_data[cmd->ringset_idx].socket);
           user_data_available_cbk(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket);
	   ipaugenblick_mark_writable(&socket_satelite_data[rte_lcore_id()][cmd->ringset_idx]);
	   ipaugenblick_mark_readable(&socket_satelite_data[rte_lcore_id()][cmd->ringset_idx]);
...
//           ipaugenblick_log(IPAUGENBLICK_LOG_DEBUG,"setting selector %d for socket %d\n",cmd->u.set_socket_select.socket_select,cmd->ringset_idx);
           socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].parent_idx = cmd->u.set_socket_select.socket_select;
	   socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].apppid = cmd->u.set_socket_select.pid;
	   user_data_available_cbk(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket);
           ipaugenblick_mark_writable(&socket_satelite_data[rte_lcore_id()][cmd->ringset_idx]);
	   ipaugenblick_mark_readable(&socket_satelite_data[rte_lcore_id()][cmd->ringset_idx]);
           break;
        case IPAUGENBLICK_SOCKET_TX_POOL_EMPTY_COMMAND:
           if(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket) {
               if(!socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket->buffers_available_notification_queue_present) {
                   TAILQ_INSERT_TAIL(&buffers_available_notification_socket_list_head[rte_lcore_id()], 
					socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket, 
					buffers_available_notification_queue_entry);
                   socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket->buffers_available_notification_queue_present = 1;
		   if(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket->type == SOCK_DGRAM)
		   	user_set_socket_tx_space(&g_ipaugenblick_sockets[rte_lcore_id()][socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].ringset_idx].tx_space,
						sk_stream_wspace(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket->sk));
               }
           }
	...
	case IPAUGENBLICK_CONNECT_CLIENT:
	   ...
	   if(!ipaugenblick_clients[rte_lcore_id()][cmd->ringset_idx].is_busy) {
	   	TAILQ_INSERT_TAIL(&ipaugenblick_clients_list_head[rte_lcore_id()],&ipaugenblick_clients[rte_lcore_id()][cmd->ringset_idx],queue_entry);
		ipaugenblick_clients[rte_lcore_id()][cmd->ringset_idx].is_busy = 1;
	...
	case IPAUGENBLICK_DISCONNECT_CLIENT:
	   ...
	   if(ipaugenblick_clients[rte_lcore_id()][cmd->ringset_idx].is_busy) {
	   	TAILQ_REMOVE(&ipaugenblick_clients_list_head[rte_lcore_id()],&ipaugenblick_clients[rte_lcore_id()][cmd->ringset_idx],queue_entry);
		ipaugenblick_clients[rte_lcore_id()][cmd->ringset_idx].is_busy = 0;
	   }
	   break;
	case IPAUGENBLICK_SETSOCKOPT_COMMAND:
	   if(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket) { 
	   	sock_setsockopt(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket, cmd->u.setsockopt.level, cmd->u.setsockopt.optname, cmd->u.setsockopt.optval, cmd->u.setsockopt.optlen);
	   }
	   break;
	case IPAUGENBLICK_SOCKET_SHUTDOWN_COMMAND:
	   if(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket) {
		inet_shutdown(socket_satelite_data[rte_lcore_id()][cmd->ringset_idx].socket, cmd->u.socket_shutdown.how);
...
void ipaugenblick_main_loop(__attribute__((unused)) void *dummy)
{
	...
	TAILQ_INIT(&buffers_available_notification_socket_list_head[rte_lcore_id()]);
	TAILQ_INIT(&ipaugenblick_clients_list_head[rte_lcore_id()]);
	...
	while(1) {
		...
		while(!TAILQ_EMPTY(&buffers_available_notification_socket_list_head[rte_lcore_id()])) {
			if(get_buffer_count() > 0) {
				struct socket *sock = TAILQ_FIRST(&buffers_available_notification_socket_list_head[rte_lcore_id()]);
				socket_satelite_data_t *socket_data = get_user_data(sock);
				if(socket_data->socket->type == SOCK_DGRAM)
					user_set_socket_tx_space(&g_ipaugenblick_sockets[rte_lcore_id()][socket_data->ringset_idx].tx_space,sk_stream_wspace(socket_data->socket->sk));
		//printf("%s %d %d %d %d\n",__FILE__,__LINE__,socket_data->ringset_idx,g_ipaugenblick_sockets[socket_data->ringset_idx].tx_space,sk_stream_wspace(socket_data->socket->sk));
					if(!ipaugenblick_mark_writable(socket_data)) { 
						...
						TAILQ_REMOVE(&buffers_available_notification_socket_list_head[rte_lcore_id()],sock,buffers_available_notification_queue_entry); 
					} else {
						break;
					}
			} else {
