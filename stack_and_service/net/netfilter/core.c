...
#include <specific_includes/linux/if.h>
			...
			int hook_thresh)
{
	 ...
	 */
	list_for_each_entry_continue((*elemp), head, list) {
		 ...
		 int hook_thresh)
{
	...
	elem = list_entry(&nf_hooks[pf][hook], struct nf_hook_ops, list);
next_hook:
...
int __init netfilter_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(nf_hooks); i++) {
		for (h = 0; h < NF_MAX_HOOKS; h++)
			LINUX_INIT_LIST_HEAD(&nf_hooks[i][h]);
	}
