...
#include <specific_includes/linux/bitmap.h>
...
#include <specific_includes/net/inet_hashtables.h>
...
{
	return __udp4_lib_lookup(net, saddr, sport, daddr, dport, dif, &udp_table[rte_lcore_id()]);
}
...
void udp_lib_rehash(struct sock *sk, u16 newhash)
{
	if (sk_hashed(sk)) {
		...
		if (hslot2 != nhslot2) {
			...
			spin_lock(&hslot2->lock);
//			hlist_nulls_del_init(&udp_sk(sk)->udp_portaddr_node);
			hslot2->count--;
			spin_unlock(&hslot2->lock);

			spin_lock(&nhslot2->lock);
			hlist_nulls_add_head(&udp_sk(sk)->udp_portaddr_node,
						 &nhslot2->head);
			nhslot2->count++;
