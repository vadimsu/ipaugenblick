...
#include <specific_includes/linux/notifier.h>
...
{
	LINUX_INIT_LIST_HEAD(&napi->poll_list);
	napi->gro_count = 0;
	napi->gro_list = NULL;
	napi->skb = NULL;
	napi->poll = poll;
...
void *netdev_lower_get_first_private_rcu(struct net_device *dev)
{
	struct netdev_adjacent *lower;
#if 0
	lower = list_first_or_null(&dev->adj_list.lower,
			struct netdev_adjacent, list);
	if (lower)
		return lower->private;
#endif
...
struct net_device *netdev_master_upper_dev_get_rcu(struct net_device *dev)
{
	struct netdev_adjacent *upper;
#if 0
	upper = list_first_or_null(&dev->adj_list.upper,
				       struct netdev_adjacent, list);
...
#endif
		list_add(&adj->list, dev_list);
	} else {
		list_add_tail(&adj->list, dev_list);
	}
...
int init_dummy_netdev(struct net_device *dev)
{
	...
	/* NAPI wants this */
	LINUX_INIT_LIST_HEAD(&dev->napi_list);
		...
		unsigned int txqs, unsigned int rxqs)
{
	...
	LINUX_INIT_LIST_HEAD(&dev->napi_list);
	LINUX_INIT_LIST_HEAD(&dev->unreg_list);
	LINUX_INIT_LIST_HEAD(&dev->close_list);
	LINUX_INIT_LIST_HEAD(&dev->link_watch_list);
	LINUX_INIT_LIST_HEAD(&dev->adj_list.upper);
	LINUX_INIT_LIST_HEAD(&dev->adj_list.lower);
	LINUX_INIT_LIST_HEAD(&dev->all_adj_list.upper);
	LINUX_INIT_LIST_HEAD(&dev->all_adj_list.lower);
...
int __init net_dev_init(void)
{
	...
	LINUX_INIT_LIST_HEAD(&ptype_all);
	for (i = 0; i < PTYPE_HASH_SIZE; i++)
		LINUX_INIT_LIST_HEAD(&ptype_base[i]);

	LINUX_INIT_LIST_HEAD(&offload_base);
	...
	for_each_possible_cpu(i) {
		...
		LINUX_INIT_LIST_HEAD(&sd->poll_list);
		sd->output_queue_tailp = &sd->output_queue;
#ifdef CONFIG_RPS
