...
#include <specific_includes/net/request_sock.h>
...
static LINUX_LIST_HEAD(proto_list);
...
int sysctl_optmem_max __read_mostly = 1024;
...
{
	int idx = prot->inuse_idx;
	int res = 0;

	res += net->core.inuse->val[idx];
...
{
	if (alloc_slab) {
		prot->slab = kmem_cache_create(prot->name, prot->obj_size, SOCK_CACHE_SIZE,
		...
		if (prot->rsk_prot != NULL) {
			...
			prot->rsk_prot->slab = kmem_cache_create(prot->rsk_prot->slab_name,
								 prot->rsk_prot->obj_size, SOCK_CACHE_SIZE,
