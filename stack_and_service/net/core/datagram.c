				    ...
				    int *peeked, int *off, int *err)
{
	...
	do {
		...
		if (!timeo)
			goto no_packet;
	} while (/*!wait_for_more_packets(sk, err, &timeo, last)*/0);
	return NULL;
			    ...
			    struct iovec *to, int len)
{
	...
	if(rte_pktmbuf_adj(skb->header_mbuf,skb->data - rte_pktmbuf_mtod(skb->header_mbuf,unsigned char *)) == NULL) {
		ipaugenblick_log(IPAUGENBLICK_LOG_ERR,"CANNOT ADJUST MBUF %s %d %d %d %d %p %p %d\n",__FILE__,__LINE__,offset,len,rte_pktmbuf_data_len(skb->header_mbuf),
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		if ((copy = end - offset) > 0) {
			int err;
			u8  *vaddr;
			struct rte_mbuf *mbuf = skb_frag_page(frag);
			...
			kunmap(page);
#else
			err = memcpy_toiovec(to, mbuf,frag->page_offset +
					     offset - start, copy);
#endif
			if (err)
				goto fault;
...
{
	ipaugenblick_log(IPAUGENBLICK_LOG_INFO,"skb_copy_datagram_iovec_failed %"PRIu64"\n",skb_copy_datagram_iovec_failed);
	ipaugenblick_log(IPAUGENBLICK_LOG_INFO,"skb_copy_datagram_iovec_called %"PRIu64"\n",skb_copy_datagram_iovec_called);
	ipaugenblick_log(IPAUGENBLICK_LOG_INFO,"skb_copy_csum_datagram_iovec_failed %"PRIu64"\n",skb_copy_csum_datagram_iovec_failed);
	ipaugenblick_log(IPAUGENBLICK_LOG_INFO,"skb_copy_csum_datagram_iovec_called %"PRIu64"\n",skb_copy_csum_datagram_iovec_called);
}
				 ...
				 int len)
{
	...
	if (copy > 0) {
		if (copy > len)
			copy = len;
		if (memcpy_fromiovecend2(skb->data + offset, from, from_offset,
					copy))
			goto fault;
		if ((len -= copy) == 0)
			return 0;
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		if ((copy = end - offset) > 0) {
			...
			if (copy > len)
				copy = len;
			vaddr = kmap(page);
			err = memcpy_fromiovecend2(vaddr + frag->page_offset +
						  offset - start,
...
#endif /* NO AF_PACKET & AF_UNIX */
#if 0 /* no cases currently to calc checksum */
				      ...
				      __wsum *csump)
{
	...
	if (copy > 0) {
		...
		if (copy > len)
			copy = len;
		*csump = csum_and_to_user(skb->data + offset, copy,
					       *csump, &err);
		if (err)
			goto fault;
		if ((len -= copy) == 0)
...
#endif /* no cases currently to calc checksum */
...
//retrieve data from skb, csum and copy fragments 
#if 0
	if (!chunk)
		return 0;
	if(iov->head == NULL) {
...
//		iov++;
	...
	if (iov->head.pkt.data_len < chunk) {
