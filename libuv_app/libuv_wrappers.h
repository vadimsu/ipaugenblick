...
int libuv_app_bind(int fd,const struct sockaddr *addr,int addr_len);

int libuv_app_connect(int fd,const struct sockaddr *addr,int addr_len);

int libuv_app_listen(int fd,int backlog);

int libuv_app_close(int fd);

int libuv_app_setsockopt(int fd,int scope,int optname,const void *val,int valsize);
